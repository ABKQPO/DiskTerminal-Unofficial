...
public class SuperInputBusAdapter implements GTMachineAdapter {

    @Override
    public String getId() {
        return "gtnl_super_input_bus";
    }

    @Override
    public boolean supports(MetaTileEntity metaTileEntity) {
        return GTMachineReflectionHelper.hasClassName(metaTileEntity, GTMachineClassNames.SUPER_INPUT_BUS_ME);
    }
    ...
    public Optional<GTMachineScanResult> scan(IGridNode node, MetaTileEntity metaTileEntity,
        ...
        ItemStack[] inventory = GTMachineReflectionHelper.readItemStackArrayField(metaTileEntity, "mInventory")
            .orElse(null);
        if (inventory == null || inventory.length < slotCount * 2) {
            return Optional.empty();
        }
        ...
        for (int slot = 0; slot < slotCount; slot++) {
            ItemStack config = inventory[slot];
            ItemStack preview = inventory[slot + slotCount];
            configs[slot] = ItemStacks.isEmpty(config) ? null : config.copy();
            extracted[slot] = ItemStacks.isEmpty(preview) ? null : preview.copy();
