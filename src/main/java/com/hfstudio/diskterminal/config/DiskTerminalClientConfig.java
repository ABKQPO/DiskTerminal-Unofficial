...
/**
 * Client-side configuration for Disk Terminal GUI preferences.
 * Uses Forge's Configuration system for proper config file management.
 */
public class DiskTerminalClientConfig {

    private static final String CONFIG_FILE = "disk_terminal_client.cfg";
    ...
    private static DiskTerminalClientConfig instance;
    ...
    private DiskTerminalClientConfig() {
        File configFile = new File(Minecraft.getMinecraft().mcDataDir, "config/" + CONFIG_FILE);
        this.config = new Configuration(configFile);

        // Settings category (shown in config GUI)
        config.setCategoryComment(
            CATEGORY_SETTINGS,
            "User-configurable settings for Disk Terminal rendering and behavior.");
        config.setCategoryLanguageKey(CATEGORY_SETTINGS, "config.disk_terminal.config.client.settings");

        this.maxHighlightDistanceProperty = config.get(
            CATEGORY_SETTINGS,
            "maxHighlightDistance",
            -1,
            "Maximum distance (in blocks) from the player for block highlighting.\n"
                + "Set to -1 for unlimited. Blocks beyond this distance will not be highlighted.",
            -1,
            10000);
        this.maxHighlightDistanceProperty
            .setLanguageKey("config.disk_terminal.config.client.settings.max_highlight_distance");
        this.maxHighlightDistance = this.maxHighlightDistanceProperty.getInt();

        this.highlightDurationProperty = config.get(
            CATEGORY_SETTINGS,
            "highlightDuration",
            15,
        ...
        this.arrowScalePercentProperty = config.get(
            ...
            "Scale percentage for directional arrows pointing to distant highlights.\n"
                + "Larger values make arrows bigger, smaller values make them smaller.",
            10,
            1000);
        this.arrowScalePercentProperty.setLanguageKey("config.disk_terminal.config.client.settings.arrow_scale");
        this.arrowScalePercent = this.arrowScalePercentProperty.getInt();

        this.textScalePercentProperty = config.get(
            CATEGORY_SETTINGS,
            "textScalePercent",
            100,
            "Scale percentage for distance text on directional arrows.",
            10,
            1000);
        this.textScalePercentProperty.setLanguageKey("config.disk_terminal.config.client.settings.text_scale");
        this.textScalePercent = this.textScalePercentProperty.getInt();

        this.adaptiveTextScaleProperty = config.get(
            CATEGORY_SETTINGS,
            "adaptiveTextScale",
            true,
        ...
        this.adaptiveTextScaleProperty
            .setLanguageKey("config.disk_terminal.config.client.settings.adaptive_text_scale");
        this.adaptiveTextScale = this.adaptiveTextScaleProperty.getBoolean();

        this.adaptiveTextScaleMinPercentProperty = config.get(
            CATEGORY_SETTINGS,
            "adaptiveTextScaleMinPercent",
            100,
            "Minimum text scale multiplier (in percent) when using adaptive text scaling.",
            10,
            1000);
        this.adaptiveTextScaleMinPercentProperty
            .setLanguageKey("config.disk_terminal.config.client.settings.adaptive_text_scale_min");
        this.adaptiveTextScaleMinPercent = this.adaptiveTextScaleMinPercentProperty.getInt();

        this.adaptiveTextScaleMaxPercentProperty = config.get(
            CATEGORY_SETTINGS,
            "adaptiveTextScaleMaxPercent",
            200,
            "Maximum text scale multiplier (in percent) when using adaptive text scaling.",
            10,
            1000);
        this.adaptiveTextScaleMaxPercentProperty
            .setLanguageKey("config.disk_terminal.config.client.settings.adaptive_text_scale_max");
        ...
        this.selectedTabProperty = config.get(
            CATEGORY_GUI,
            "selectedTab",
            0,
            "The currently selected tab in the Disk Terminal GUI (0=Terminal, 1=Inventory, 2=Partition)");
        this.selectedTabProperty.setLanguageKey("config.disk_terminal.gui.selectedTab");
        this.selectedTab = this.selectedTabProperty.getInt();

        this.terminalStyleProperty = config.get(
            CATEGORY_GUI,
            "terminalStyle",
            TerminalStyle.SMALL.name(),
            "Terminal size style: SMALL (fixed 8 rows), TALL (expands to fill screen)");
        this.terminalStyleProperty.setLanguageKey("config.disk_terminal.gui.terminalStyle");
        this.terminalStyle = TerminalStyle.fromName(this.terminalStyleProperty.getString());

        this.searchFilterProperty = config
            .get(CATEGORY_GUI, "searchFilter", "", "The last search filter text used in the Disk Terminal");
        this.searchFilterProperty.setLanguageKey("config.disk_terminal.gui.searchFilter");
        this.searchFilter = this.searchFilterProperty.getString();

        this.searchModeProperty = config.get(
            CATEGORY_GUI,
            "searchMode",
            SearchFilterMode.MIXED.name(),
            "Search filter mode: INVENTORY (search contents), PARTITION (search filters), MIXED (search both)");
        this.searchModeProperty.setLanguageKey("config.disk_terminal.gui.searchMode");
        this.searchMode = SearchFilterMode.fromName(this.searchModeProperty.getString());

        this.cellSlotLimitProperty = config.get(
            CATEGORY_GUI,
            "cellSlotLimit",
            SlotLimit.UNLIMITED.name(),
            "Slot limit for cell content display: LIMIT_8, LIMIT_32, LIMIT_64, or UNLIMITED");
        this.cellSlotLimitProperty.setLanguageKey("config.disk_terminal.gui.cellSlotLimit");
        this.cellSlotLimit = SlotLimit.fromName(this.cellSlotLimitProperty.getString());

        this.busSlotLimitProperty = config.get(
            CATEGORY_GUI,
            "busSlotLimit",
            SlotLimit.UNLIMITED.name(),
            "Slot limit for storage bus content display: LIMIT_8, LIMIT_32, LIMIT_64, or UNLIMITED");
        this.busSlotLimitProperty.setLanguageKey("config.disk_terminal.gui.busSlotLimit");
        this.busSlotLimit = SlotLimit.fromName(this.busSlotLimitProperty.getString());

        this.subnetSlotLimitProperty = config.get(
            CATEGORY_GUI,
            "subnetSlotLimit",
            SlotLimit.LIMIT_64.name(),
            "Slot limit for subnet inventory content display: LIMIT_8, LIMIT_32, LIMIT_64, or UNLIMITED");
        this.subnetSlotLimitProperty.setLanguageKey("config.disk_terminal.gui.subnetSlotLimit");
        this.subnetSlotLimit = SlotLimit.fromName(this.subnetSlotLimitProperty.getString());

        this.lastViewedNetworkIdProperty = config.get(
            CATEGORY_GUI,
            "lastViewedNetworkId",
            "0",
            "The last viewed network ID (0 = main network, other = subnet ID). Stored as string to support long values.");
        this.lastViewedNetworkIdProperty.setLanguageKey("config.disk_terminal.gui.lastViewedNetworkId");
        try {
        ...
        this.subnetVisibilityProperty = config.get(
            CATEGORY_GUI,
            "subnetVisibility",
            SubnetVisibility.DONT_SHOW.name(),
            "Subnet content visibility in each tab: DONT_SHOW, SHOW_FAVORITES, or SHOW_ALL");
        this.subnetVisibilityProperty.setLanguageKey("config.disk_terminal.gui.subnetVisibility");
        ...
        config.setCategoryLanguageKey(CATEGORY_FILTERS, "config.disk_terminal.filters");
        for (CellFilter filter : CellFilter.values()) {
            // Cell filter states (tabs 0-2)
            String cellKey = filter.getConfigKey(false);
            Property cellProp = config.get(
                CATEGORY_FILTERS,
                cellKey,
                CellFilter.State.SHOW_ALL.name(),
                "Filter state for " + filter.name() + " (cells): SHOW_ALL, SHOW_ONLY, or HIDE");
            cellProp.setLanguageKey("config.disk_terminal.filter." + cellKey);
            ...
            Property busProp = config.get(
                CATEGORY_FILTERS,
                busKey,
                CellFilter.State.SHOW_ALL.name(),
                "Filter state for " + filter.name() + " (storage buses): SHOW_ALL, SHOW_ONLY, or HIDE");
            busProp.setLanguageKey("config.disk_terminal.filter." + busKey);
            filterProperties.put(busKey, busProp);
            busFilterStates.put(filter, CellFilter.State.fromName(busProp.getString()));
        }

        config.setCategoryLanguageKey(CATEGORY_GUI, "config.disk_terminal.gui");

        if (config.hasChanged()) config.save();
    }

    public static DiskTerminalClientConfig getInstance() {
        if (instance == null) instance = new DiskTerminalClientConfig();
    ...
    public SlotLimit getSlotLimitForTab(int tabIndex) {
        return switch (tabIndex) {
            case GuiConstants.TAB_SUBNETS -> subnetSlotLimit;
            case GuiConstants.TAB_TERMINAL, GuiConstants.TAB_INVENTORY, GuiConstants.TAB_PARTITION -> cellSlotLimit;
            case GuiConstants.TAB_STORAGE_BUS_INVENTORY, GuiConstants.TAB_STORAGE_BUS_PARTITION -> busSlotLimit;
            default -> SlotLimit.UNLIMITED;
        };
    }
