...
public class NetworkToolFilterUtils {
     ...
     */
    public static List<INetworkTool.FilteredCell> getFilteredCells(Map<Long, StorageInfo> storageMap,
        Map<CellFilter, CellFilter.State> activeFilters, SearchFilterMode searchMode, String searchFilter,
        boolean useAdvancedSearch, AdvancedSearchParser.SearchMatcher advancedMatcher) {
        ...
        for (StorageInfo storage : storageMap.values()) {
            for (int slot = 0; slot < storage.getSlotCount(); slot++) {
                ...
                if (!cellMatchesSearchFilter(
                    cell,
                    storage,
                    searchMode,
                    searchFilter,
                    useAdvancedSearch,
                    advancedMatcher)) continue;
     ...
     */
    public static List<StorageBusInfo> getFilteredStorageBuses(Map<Long, StorageBusInfo> storageBusMap,
        Map<CellFilter, CellFilter.State> activeFilters, SearchFilterMode searchMode, String searchFilter,
        boolean useAdvancedSearch, AdvancedSearchParser.SearchMatcher advancedMatcher) {
        ...
        for (StorageBusInfo bus : storageBusMap.values()) {
            ...
            // Then check text search filter
            if (!storageBusMatchesSearchFilter(bus, searchMode, searchFilter, useAdvancedSearch, advancedMatcher))
                continue;
     ...
     */
    public static List<INetworkTool.FilteredCell> getEmptyNonPartitionedCells(Map<Long, StorageInfo> storageMap,
        StorageType type) {
        ...
        for (StorageInfo storage : storageMap.values()) {
            for (int slot = 0; slot < storage.getSlotCount(); slot++) {
                ...
                if (cell.getCellItem()
                    .getItem() instanceof IItemCompactingCell) continue;

                // Check cell type matches requested types
                if (cell.getStorageType() != type) continue;

                // Check if empty and non-partitioned
                boolean isEmpty = cell.getContents()
                    .isEmpty()
                    || cell.getContents()
                        .stream()
                        .allMatch(ItemStacks::isEmpty);
                boolean hasPartition = cell.getPartition()
                    .stream()
     ...
     * @param searchMode    The search filter mode (affects HAS_ITEMS filter interpretation)
     */
    public static boolean cellMatchesFilters(CellInfo cell, Map<CellFilter, CellFilter.State> activeFilters,
     ...
     * @param searchMode    The search filter mode (affects HAS_ITEMS filter interpretation)
     */
    public static boolean storageBusMatchesFilters(StorageBusInfo bus, Map<CellFilter, CellFilter.State> activeFilters,
        SearchFilterMode searchMode) {
    ...
    private static boolean checkHasItemsFilter(CellInfo cell, Map<CellFilter, CellFilter.State> activeFilters,
        SearchFilterMode searchMode) {
    ...
    private static boolean checkBusHasItemsFilter(StorageBusInfo bus, Map<CellFilter, CellFilter.State> activeFilters,
        SearchFilterMode searchMode) {
        CellFilter.State state = activeFilters.getOrDefault(CellFilter.HAS_ITEMS, CellFilter.State.SHOW_ALL);
        if (state == CellFilter.State.SHOW_ALL) return true;
    ...
    private static boolean checkBusPartitionedFilter(StorageBusInfo bus,
     ...
     */
    private static boolean cellMatchesSearchFilter(CellInfo cell, StorageInfo storage, SearchFilterMode searchMode,
        String searchFilter, boolean useAdvancedSearch, AdvancedSearchParser.SearchMatcher advancedMatcher) {
     ...
     */
    private static boolean storageBusMatchesSearchFilter(StorageBusInfo bus, SearchFilterMode searchMode,
        String searchFilter, boolean useAdvancedSearch, AdvancedSearchParser.SearchMatcher advancedMatcher) {
