...
public class MassPartitionCellTool implements INetworkTool {
    ...
    public String getName() {
        return I18n.format("gui.disk_terminal.networktools.mass_partition_cell.name");
    }

    @Override
    public String getDescription() {
        return I18n.format("gui.disk_terminal.networktools.mass_partition_cell.desc");
    }

    @Override
    public List<String> getHelpLines() {
        List<String> lines = new ArrayList<>();
        lines.add(I18n.format("gui.disk_terminal.networktools.mass_partition_cell.help.1"));
        lines.add("");
        lines.add(I18n.format("gui.disk_terminal.networktools.mass_partition_cell.help.2"));
    ...
    public ToolPreviewInfo getPreview(ToolContext context) {
        ...
        if (totalCount == 0) {
            tooltipLines.add(I18n.format("gui.disk_terminal.networktools.no_targets"));
        } else {
            tooltipLines.add(I18n.format("gui.disk_terminal.networktools.target_breakdown"));
    ...
    public String getExecutionError(ToolContext context) {
        List<FilteredCell> cells = getPartitionableCells(context);
        if (cells.isEmpty()) return I18n.format("gui.disk_terminal.networktools.error.no_cells");
    ...
    public String getConfirmationMessage(ToolContext context) {
        ...
        return I18n.format("gui.disk_terminal.networktools.mass_partition_cell.confirm", count);
    }
