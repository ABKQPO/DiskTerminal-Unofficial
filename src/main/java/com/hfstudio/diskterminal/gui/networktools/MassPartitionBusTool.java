...
public class MassPartitionBusTool implements INetworkTool {
    ...
    public String getName() {
        return I18n.format("gui.disk_terminal.networktools.mass_partition_bus.name");
    }

    @Override
    public String getDescription() {
        return I18n.format("gui.disk_terminal.networktools.mass_partition_bus.desc");
    }

    @Override
    public List<String> getHelpLines() {
        List<String> lines = new ArrayList<>();
        lines.add(I18n.format("gui.disk_terminal.networktools.mass_partition_bus.help.1"));
        lines.add("");
        lines.add(I18n.format("gui.disk_terminal.networktools.mass_partition_bus.help.2"));
    ...
    public String getExecutionError(ToolContext context) {
        List<StorageBusInfo> buses = context.getFilteredStorageBuses();
        if (buses.isEmpty()) return I18n.format("gui.disk_terminal.networktools.error.no_buses");
    ...
    public String getConfirmationMessage(ToolContext context) {
        int count = context.getFilteredStorageBuses()
            .size();

        return I18n.format("gui.disk_terminal.networktools.mass_partition_bus.confirm", count);
    }
