...
import com.hfstudio.diskterminal.network.PacketNetworkToolAction;
...
public class AttributeUniqueTool implements INetworkTool {
    ...
    public String getName() {
        return I18n.format("gui.disk_terminal.networktools.attribute_unique.name");
    }

    @Override
    public String getDescription() {
        return I18n.format("gui.disk_terminal.networktools.attribute_unique.desc");
    }

    @Override
    public List<String> getHelpLines() {
        List<String> lines = new ArrayList<>();
        lines.add(I18n.format("gui.disk_terminal.networktools.attribute_unique.help.1"));
        lines.add(I18n.format("gui.disk_terminal.networktools.attribute_unique.help.2"));
        lines.add(I18n.format("gui.disk_terminal.networktools.attribute_unique.help.3"));
        lines.add("");
        lines.add(I18n.format("gui.disk_terminal.networktools.attribute_unique.help.4"));
    ...
    private Map<StorageType, TypeStats> collectTypeStats(ToolContext context) {
        ...
        for (FilteredCell fc : filteredCells) {
            ...
            for (ItemStack stack : cell.getContents()) {
                if (!ItemStacks.isEmpty(stack)) stats.uniqueKeys.add(getItemKey(stack, type));
            }
        }
    ...
    public ToolPreviewInfo getPreview(ToolContext context) {
        ...
        tooltipLines.add(I18n.format("gui.disk_terminal.networktools.attribute_unique.preview.title"));
        tooltipLines.add("");
        ...
        for (StorageType type : StorageType.values()) {
            ...
            String typeName = type.name()
                .toLowerCase();
            String typeLabel = I18n.format("gui.disk_terminal.networktools.attribute_unique.preview.type." + typeName);
        ...
        if (hasAnyError) {
            tooltipLines.add("");
            tooltipLines
                .add("§c" + I18n.format("gui.disk_terminal.networktools.attribute_unique.error.not_enough_cells"));
        }
    ...
    public String getExecutionError(ToolContext context) {
        List<FilteredCell> filteredCells = context.getFilteredCells();
        if (filteredCells.isEmpty()) return I18n.format("gui.disk_terminal.networktools.error.no_cells");
        ...
        if (!hasAnyContent) return I18n.format("gui.disk_terminal.networktools.error.no_items");
        ...
        if (hasError) {
            return I18n.format("gui.disk_terminal.networktools.attribute_unique.error.not_enough_cells_by_type")
                + errorBuilder;
        }
    ...
    public String getConfirmationMessage(ToolContext context) {
        ...
        return I18n.format("gui.disk_terminal.networktools.attribute_unique.confirm", totalUnique, totalCells);
    }

    @Override
    public void execute(ToolContext context) {
        DiskTerminalNetwork.INSTANCE.sendToServer(new PacketNetworkToolAction(TOOL_ID, context.getActiveFilters()));
    }
}
