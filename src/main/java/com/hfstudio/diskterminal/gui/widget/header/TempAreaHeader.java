...
public class TempAreaHeader extends AbstractHeader {
    ...
    protected void drawIcon() {
        ...
        if (!ItemStacks.isEmpty(icon)) renderItemStack(icon, slotX, y);
    }
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        if (drawConnector) {
            Gui.drawRect(
                TREE_LINE_X,
                y + GuiConstants.HEADER_CONNECTOR_Y_OFFSET,
                TREE_LINE_X + 1,
                y + GuiConstants.ROW_HEIGHT,
    ...
    private void drawTempName(int mouseX, int mouseY) {
        ...
        if (hasCell) {
            ...
            name = I18n.format("gui.disk_terminal.temp_area.drop_cell");
        }
    ...
    public List<String> getTooltip(int mouseX, int mouseY) {
        ...
        if (sendButtonHovered) {
            return Collections.singletonList(I18n.format("gui.disk_terminal.temp_area.send.tooltip"));
        }
    ...
    public ItemStack getHoveredItemStack(int mouseX, int mouseY) {
        ...
        if (cellSlotHovered) {
            ItemStack icon = iconSupplier != null ? iconSupplier.get() : null;
            if (!ItemStacks.isEmpty(icon)) return icon;
        }
