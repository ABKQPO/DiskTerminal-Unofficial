package com.hfstudio.diskterminal.gui.rename;
 ...
 * and sending rename packets directly via {@link DiskTerminalNetwork}.
...
public class InlineRenameManager {
        ...
        switch (target.getRenameTargetType()) {
            case STORAGE:
                DiskTerminalNetwork.INSTANCE
                    .sendToServer(PacketRenameAction.renameStorage(target.getRenameId(), newName));
                break;
            case CELL:
    ...
    public boolean handleKey(char typedChar, int keyCode) {
        ...
        if (keyCode == Keyboard.KEY_BACK && editingCursorPos > 0) {
            editingText = editingText.substring(0, editingCursorPos - 1) + editingText.substring(editingCursorPos);
            editingCursorPos--;
            return true;
        }

        if (keyCode == Keyboard.KEY_DELETE && editingCursorPos < editingText.length()) {
            editingText = editingText.substring(0, editingCursorPos) + editingText.substring(editingCursorPos + 1);
            return true;
        }
        ...
        if (typedChar >= 32 && typedChar < 127 && editingText.length() < MAX_NAME_LENGTH) {
            editingText = editingText.substring(0, editingCursorPos) + typedChar
    ...
    private static boolean isSameTarget(Renameable a, Renameable b) {
        ...
        return a.getRenameTargetType() == b.getRenameTargetType() && a.getRenameId() == b.getRenameId()
