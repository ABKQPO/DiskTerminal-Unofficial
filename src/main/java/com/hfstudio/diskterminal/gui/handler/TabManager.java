...
import com.hfstudio.diskterminal.gui.widget.tab.CellContentTabWidget;
import com.hfstudio.diskterminal.gui.widget.tab.GuiContext;
import com.hfstudio.diskterminal.gui.widget.tab.NetworkToolsTabWidget;
import com.hfstudio.diskterminal.gui.widget.tab.StorageBusTabWidget;
import com.hfstudio.diskterminal.gui.widget.tab.SubnetOverviewTabWidget;
import com.hfstudio.diskterminal.gui.widget.tab.TempAreaTabWidget;
import com.hfstudio.diskterminal.gui.widget.tab.TerminalTabWidget;
...
public class TabManager {
    ...
    private int findFirstEnabledTab() {
        if (!DiskTerminalServerConfig.isInitialized()) return GuiConstants.TAB_TERMINAL;
     ...
     */
    public void initWidgets(FontRenderer fontRenderer, RenderItem itemRender, int guiLeft, int guiTop, int rowsVisible,
        GuiContext context) {
        // Create tab widgets
        this.terminalWidget = new TerminalTabWidget(fontRenderer, itemRender);
        CellContentTabWidget inventoryWidget = new CellContentTabWidget(
            SlotsLine.SlotMode.CONTENT,
            fontRenderer,
            itemRender);
        CellContentTabWidget partitionWidget = new CellContentTabWidget(
            SlotsLine.SlotMode.PARTITION,
            fontRenderer,
            itemRender);
        TempAreaTabWidget tempAreaWidget = new TempAreaTabWidget(fontRenderer, itemRender);
        StorageBusTabWidget storageBusInventoryWidget = new StorageBusTabWidget(
            SlotsLine.SlotMode.CONTENT,
            fontRenderer,
            itemRender);
        StorageBusTabWidget storageBusPartitionWidget = new StorageBusTabWidget(
            SlotsLine.SlotMode.PARTITION,
            fontRenderer,
            itemRender);
        ...
        this.tabWidgets = new AbstractTabWidget[] { terminalWidget, // TAB_TERMINAL (0)
    ...
    public void switchToTab(int newTab) {
        ...
        if (scrollAccessor != null) {
            ...
            TabStateManager.getInstance()
                .setScrollPosition(oldTabType, scrollAccessor.getCurrentScroll());
        }
        ...
        if (newTab >= 0) DiskTerminalClientConfig.getInstance()
        ...
        if (scrollAccessor != null) {
            ...
            int savedScroll = TabStateManager.getInstance()
                .getScrollPosition(newTabType);
    ...
    public String getTabTooltip(int tab) {
        ...
        if (DiskTerminalServerConfig.isInitialized() && !DiskTerminalServerConfig.getInstance()
            .isTabEnabled(tab)) {
            return baseTooltip + " " + I18n.format("gui.disk_terminal.tab.disabled");
        }
     ...
     */
    public void drawTabs(int guiLeft, int offsetX, int offsetY, int mouseX, int mouseY, RenderItem itemRender,
        Minecraft mc) {
        TabRenderingHandler.TabRenderContext ctx = new TabRenderingHandler.TabRenderContext(
            ...
            mouseX,
            mouseY,
            GuiConstants.TAB_WIDTH,
            GuiConstants.TAB_HEIGHT,
            GuiConstants.TAB_Y_OFFSET,
            currentTab,
            itemRender,
            mc);
        ...
        TabRenderingHandler.TabIconProvider iconProvider = new TabRenderingHandler.TabIconProvider() {
            ...
            public ItemStack getStorageBusIcon() {
                if (tabIconStorageBus == null) {
                    tabIconStorageBus = AEApi.instance()
                        .definitions()
                        .parts()
                        .storageBus()
                        .maybeStack(1)
            ...
            public ItemStack getInventoryIcon() {
                if (tabIconInventory == null) {
                    tabIconInventory = AEApi.instance()
                        .definitions()
                        .blocks()
                        .chest()
                        .maybeStack(1)
            ...
            public ItemStack getPartitionIcon() {
                if (tabIconPartition == null) {
                    tabIconPartition = AEApi.instance()
                        .definitions()
                        .blocks()
                        .cellWorkbench()
                        .maybeStack(1)
