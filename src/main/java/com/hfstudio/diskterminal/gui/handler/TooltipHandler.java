...
public class TooltipHandler {
    ...
    public static void drawTooltips(TooltipContext ctx, TooltipRenderer renderer, int mouseX, int mouseY) {
        ...
        if (ctx.subnetVisibilityButton != null && ctx.subnetVisibilityButton.visible
            && ctx.subnetVisibilityButton.isMouseOver()) {
        ...
        if (ctx.hasSearchError && ctx.searchErrorMessage != null) {
            boolean hoveringSearchField = mouseX >= ctx.searchFieldX && mouseX < ctx.searchFieldX + ctx.searchFieldWidth
                && mouseY >= ctx.searchFieldY
                && mouseY < ctx.searchFieldY + ctx.searchFieldHeight;
            if (hoveringSearchField) {
                List<String> errorTooltip = new ArrayList<>();
                errorTooltip.add("§c" + I18n.format("gui.disk_terminal.search_error"));
        ...
        if (PriorityFieldManager.getInstance()
            .isMouseOverField(mouseX, mouseY)) {
            renderer.drawHoveringText(
                Collections.singletonList(I18n.format("gui.disk_terminal.priority.tooltip")),
                mouseX,
                mouseY);
        ...
        if (ctx.filterPanelManager != null) {
            ...
            if (slotBtn != null && slotBtn.visible
                && mouseX >= slotBtn.xPosition
                && mouseX < slotBtn.xPosition + slotBtn.width
                && mouseY >= slotBtn.yPosition
                && mouseY < slotBtn.yPosition + slotBtn.height) {
