...
public class GuiConstants {
    ...
    /** Child window texture resource location. */
    public static final ResourceLocation CHILD_TEXTURE = new ResourceLocation(
        "disk_terminal",
        "textures/guis/child.png");
    ...
    public static void drawTerminalUpperBorderLine(int x, int y, int width) {
        drawHorizontalTiledTerminalSprite(x, y, 21, 51, 164, 1, width, 1, 4);
    }

    public static void drawChildWindowBackground(int x, int y, int width, int height) {
        drawNineSlicedTexture(CHILD_TEXTURE, x, y, 0, 0, 128, 128, width, height, 4, 256, 256);
    }

    public static void drawChildWindowButton(int x, int y, int width, int height) {
        drawNineSlicedTexture(ATLAS_TEXTURE, x, y, 60, 40, 10, 10, width, height, 2, ATLAS_WIDTH, ATLAS_HEIGHT);
    }

    public static void drawChildWindowButtonHover(int x, int y, int width, int height) {
        drawNineSlicedTexture(ATLAS_TEXTURE, x, y, 60, 50, 10, 10, width, height, 2, ATLAS_WIDTH, ATLAS_HEIGHT);
    }

    public static void drawNineSlicedTexture(ResourceLocation texture, int x, int y, int texX, int texY,
        int sourceWidth, int sourceHeight, int width, int height, int edgeSize, int textureWidth, int textureHeight) {
        if (width <= 0 || height <= 0) return;

        int horizontalEdge = Math.min(edgeSize, width / 2);
        int verticalEdge = Math.min(edgeSize, height / 2);
        ...
        int centerSourceWidth = Math.max(0, sourceWidth - edgeSize * 2);
        int centerSourceHeight = Math.max(0, sourceHeight - edgeSize * 2);
        int centerWidth = Math.max(0, width - horizontalEdge * 2);
        int centerHeight = Math.max(0, height - verticalEdge * 2);
        int rightSourceX = texX + sourceWidth - edgeSize;
        int bottomSourceY = texY + sourceHeight - edgeSize;
    ...
    /**
     * 1.7.10 replacement for the 1.8+ {@code Gui.drawScaledCustomSizeModalRect}. Blits a region
     * of a texture (sampled at {@code u,v} sized {@code uWidth,vHeight} out of a {@code tileWidth} by
