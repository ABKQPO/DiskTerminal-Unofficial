 ...
 * sized according to {@link DiskTerminalServerConfig#getMaxChunkBytes()}, and sends the chunks
...
public class ChunkedNBTSender {
    ...
    public static void send(EntityPlayerMP player, String channel, PayloadMode mode, NBTTagCompound data) {
        ...
        try {
            ...
            DiskTerminal.LOG.error("Failed to compress NBT payload for channel " + channel, e);
            return;
        }

        int maxChunkBytes = DiskTerminalServerConfig.getInstance()
            .getMaxChunkBytes();
        ...
        for (int i = 0; i < totalChunks; i++) {
            ...
            DiskTerminalNetwork.INSTANCE
                .sendTo(new PacketNBTChunk(channel, sessionId, i, totalChunks, mode, slice), player);
        }
    }

    private static byte[] compress(NBTTagCompound data) throws IOException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try (GZIPOutputStream gzos = new GZIPOutputStream(baos); DataOutputStream dos = new DataOutputStream(gzos)) {
